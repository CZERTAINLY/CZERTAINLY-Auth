...
namespace Czertainly.Auth.Data.Repositiories
{
    public class PermissionRepository : BaseRepository<Permission>, IPermissionRepository
    {
        ...
        public async Task<List<Permission>> GetUserPermissions(Guid userUuid)
        {
            ...
            return await _dbSet.IgnoreAutoIncludes().Include(p => p.Resource).Include(p => p.Action).Include(p => p.Role).ThenInclude(r => r.Users.Where(u => u.Uuid == userUuid)).Where(p => p.Role.Users.All(u => u.Uuid == userUuid)).OrderByDescending(p => p.ResourceId).ThenByDescending(p => p.ActionId).ThenByDescending(p => p.ObjectUuid).ToListAsync();
        }
    }
}
